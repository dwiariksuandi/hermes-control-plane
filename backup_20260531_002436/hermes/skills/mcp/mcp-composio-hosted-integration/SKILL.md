...
- **Timeout or hanging when using `hermes chat` to invoke Composio tools for file uploads/downloads**: `hermes chat` has request-level time limits and is poor fit for bulk transfer. Use it for discovery, metadata, or small smoke-test artifacts only. For files larger than a few MB, prefer the provider’s resumable upload tool if directly accessible; otherwise pivot to a dedicated sync tool such as `rclone` after two failed wrapper attempts.