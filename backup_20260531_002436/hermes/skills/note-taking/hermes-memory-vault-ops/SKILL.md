...
## Hygiene & Retention Standards (CleanSlate)
- Always maintain structured evidence in `vault/dev/backup-manifests/` and `vault/dev/retention-reports/`.
- Use `hygiene_manager.py` for automated cleanup (7d local backups, 90d manifests, 30d logs).
- Always generate a `.bak` before any deletion (SOUL.md Law 6).
- Record retention events in `vault/self-learning/learnings.md` and `vault/dev/CLEAN_SLATE_LEDGER.md`.
