...
5. **Build the artifact**
   ...
   - *For ops dashboards with a local backend, see "Ops Dashboard Integration Patterns" below and the reference file `references/ops-dashboard-fastapi-websocket.md`.*
...
## Ops Dashboard Integration Patterns (FastAPI + Vanilla JS)

When artifact is an operational dashboard backed by a local API, apply these rules:

1. **Use explicit API/WS base constants**
   - Define once in JS:
     - `const API_BASE = "http://localhost:8000"`
     - `const WS_BASE = "ws://localhost:8000"`
   - Use absolute paths for all fetch/WebSocket calls.
   - This avoids route breakage when opening HTML from non-root/static origins.

2. **Do not rely on implicit global `event` in tab handlers**
   - If using inline handlers, pass event explicitly:
     - `onclick="switchTab(event, 'fleet')"`
   - Function signature must accept event argument:
     - `function switchTab(evt, name) { ... }`
   - Pitfall: `event.currentTarget` without explicit event can fail and break tab navigation.

3. **Prefer one realtime channel for telemetry**
   - If backend has WebSocket stream, avoid duplicate polling loops for same data.
   - Keep a single source for logs/system-health updates to reduce jitter and race conditions.

4. **Backend file explorer endpoints need path traversal guard**
   - For `/api/vault/file?path=...`, resolve path and enforce root-prefix check.
   - Return 403 for traversal attempts (`../../...`).

5. **Verification checklist for ops dashboards**
   - Check these endpoints physically: `/api/status`, `/api/fleet`, `/api/vault/tree`, `/api/vault/file`, `/api/system-health`.
   - Confirm websocket endpoint exists in backend (`/ws/telemetry`).
   - Confirm frontend has target tab IDs and matching nav handlers.

See also: `references/ops-dashboard-fastapi-websocket.md`
