...
### Alternative: Hosted MCP (OAuth) — No Local Script Needed

Instead of running a local Python script, you can use Composio's **hosted MCP endpoint** directly. This avoids maintaining a `run_server.py` and works with any Composio account.

1.  **Get your OAuth connection** from [composio.dev](https://composio.dev):
    - Go to **MCP Servers** → **Add MCP Server** → copy the `url`.
2.  **Configure** `~/.hermes/config.yaml`:
    ```yaml
    mcp_servers:
      composio:
        url: https://connect.composio.dev/mcp   # Your Composio-provided URL
        auth: oauth                              # Triggers device-code auth flow
    ```
3.  **Restart Hermes**. On first use, Hermes prompts you to open a link, log in, and authorize — the OAuth token is stored in `auth.json` automatically.
4.  **Verify** with `hermes mcp list` — look for `composio` in the server list.

**Which approach to choose?**

| Approach | Pros | Cons |
|----------|------|------|
| **Local script** (`command` + `args`) | Full control over tool selection; works offline | Requires venv + SDK maintenance |
| **Hosted MCP** (`url` + `auth`) | No script/venv to maintain; always latest tools | Needs internet; OAuth may expire |

**⚠️ Config conflict:** You can configure either a **local** (`command` + `args`) or **hosted** (`url` + `auth`) server for Composio, not both under the same server name. If you switch approaches, remove the old entry from `config.yaml` entirely before adding the new one. A stale leftover entry blocks Hermes from loading the other.
...
- `venv-pip-bootstrap`: For managing isolated Python environments
