...
### Path A — Hosted HTTP MCP endpoint (ONLY WORKING PATH)
...
4. **Set enabled explicitly**:
   - `hermes config set mcp_servers.composio.enabled true`
5. **Verify registration**:
   - `hermes mcp list` (must show `composio` enabled with HTTPS transport)
6. **Complete auth in fresh terminal**:
   - `hermes mcp login composio`

### Path B — Legacy/local bridge path (BROKEN)
DO NOT USE. In `composio-core` >= 0.7.21, the `composio.core.mcp` module is removed (`ModuleNotFoundError`). Local bridge scripts will fail. Always use Path A.
...
### Critical: Config must be minimal for hosted endpoint
The hosted Composio MCP endpoint (`https://connect.composio.dev/mcp` with `auth: oauth`) requires **only three keys** in `config.yaml`:
```yaml
composio:
  ...
  enabled: true
```
Do NOT add `auth_config`, `client_id`, `client_secret`, `allowed_tools`, `command`, `args`, or `env` — these break the hosted connection. The OAuth flow is handled entirely by Composio's remote endpoint; Hermes negotiates it via `hermes mcp login composio`.

### Critical: Local bridge (Path B) is broken as of composio-core 0.7.21
The `composio.core.mcp.server` module does not exist in composio-core 0.7.21. Any local bridge script importing from `composio.core.mcp` will fail with `ModuleNotFoundError`. **Do not use Path B until the SDK provides a working MCP server module.** The hosted endpoint (Path A) works reliably and requires no local bridge.
...
## Pitfalls
- **Do NOT add `auth_config` or `allowed_tools` to `mcp_servers.composio` in config.yaml when using the hosted HTTP endpoint (`https://connect.composio.dev/mcp`).** These keys are for local bridge setups or SDK server creation — adding them to a hosted MCP config causes the server to disconnect on reload. The correct hosted config is just `url`, `auth: oauth`, and optionally `enabled: true`. This error was observed in May 2026 session: adding placeholder `auth_config` + `allowed_tools` caused Hermes to drop Composio from the active MCP server list on reload.
