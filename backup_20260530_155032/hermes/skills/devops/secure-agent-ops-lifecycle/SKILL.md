...
- Do not run shell commands through `execute_code`. Use `terminal` for shell, `execute_code` only for Python orchestration. Symptom: `SyntaxError` at line beginning with `python3 ...` inside `/tmp/hermes_sandbox_*/script.py`.

### Secret Encryption Before Sync

**Pattern:** When backing up sensitive files (`.env`, `auth.json`, `.db`), encrypt them before replication. The private key must **never** be included in the backup archive.

**Workflow:**
1.  Generate an `age` keypair. Store the **private key** securely (e.g., `/home/hiryu/.hermes/hermes_keys/hermes-backup.key` with `600` perms). Keep it **out of the backup**.
2.  Encrypt secrets using the public key:
    ```bash
    age -p -i /home/hiryu/.hermes/hermes_keys/public.txt /home/hiryu/.hermes/.env > /tmp/backup/stage/encrypted_secrets/.env.age
    ```
3.  Create a consistent snapshot of active files (e.g., SQLite DB) before encryption.
4.  Remove plaintext secrets from the staging area after encryption.
5.  Add `hermes_keys/` to `.gitignore` to prevent accidental key inclusion.
6.  Replicate the encrypted archive. Verify with checksums.

**Rationale:** This ensures the backup can be safely stored or transmitted. Even if the backup is compromised, secrets remain protected by the encryption key that was excluded from the archive.

## Tool Routing Guard (Hermes)
Before running backup/push flows, apply this routing check:
1. If instruction starts with shell syntax (`python3`, `git`, pipes, redirects), run with `terminal`.
2. If task needs loops/conditionals across multiple tool calls, use `execute_code` with valid Python only.
3. On two consecutive tool-class errors, pivot immediately (ANTI-LOOP): switch tool class rather than retrying same call.
